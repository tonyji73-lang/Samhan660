...
func _run() -> void:
 if FileAccess.get_file_as_string("res://ai_military_planning.gd").contains("const Preparation="): push_error("Before audit requires preserved baseline code; refusing to overwrite baseline with final code"); quit(2); return
 DIR="res://.godot/completion-training-before/"; DirAccess.make_dir_recursive_absolute(DIR)
 var rows: Array=[]
