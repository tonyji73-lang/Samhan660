...
func click(button: Control) -> void:
	var in_options: bool=is_instance_valid(ui) and is_instance_valid(ui._comparison_panel) and ui._comparison_panel.is_ancestor_of(button)
	if in_options and not ui._comparison_panel.visible:
		await super.click(ui.buttons.view_options)
	var ancestor := button.get_parent()
	while ancestor != null:
		if ancestor is ScrollContainer: ancestor.ensure_control_visible(button)
		ancestor=ancestor.get_parent()
	await super.click(button)
	if in_options and ui._comparison_panel.visible:
		await super.click(ui.buttons.view_options)
