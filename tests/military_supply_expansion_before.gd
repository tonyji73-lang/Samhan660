...
func metrics(faction: String) -> Dictionary:
 ...
 return row
func _run() -> void:
 if P.new().get_script().get_script_constant_map().has("Network"):
  push_error("Run with both campaign and planner from the preserved starting snapshot; see report."); quit(2); return
 DIR="res://.godot/supply-expansion-before-final/"
