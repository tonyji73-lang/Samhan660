...
func _run() -> void:
	...
	for mode: String in ["common","stock_transport","regional"]:
		...
		var city: String="sabi" if mode!="regional" else "geumgwan"
		var s: Dictionary=c.strategy_state
		# Controlled available-infrastructure fixture, not normal-play procurement.
		s.province_buildings[city].smelter=1; s.province_buildings[city].forge=1
		s.faction_research[name].basic_smelting=1; s.faction_research[name].swordsmithing=1
		s["iron_ai"]={"months":{},"log":[],"last":{},"hubs":{f:city}}
		Economy.post(s,f,2000,"explicit_ai_fixture",stamp())
		if mode=="stock_transport":
			s.city_inventory.ungjin.iron=20
			Production.set_enabled(s,c.provinces,city,"iron_sword",name,true,c.scenario_id,c.iron_supply_rules)
		c.run_enemy_ai_turns()
		if mode=="stock_transport":
			check(c.Supply.ensure(s).orders.values().any(func(o): return o.target==city and o.cargo.iron>0),"actual AI compares and dispatches existing local iron stock")
		else:
			check(s.city_production[city]["iron_supply" if mode=="regional" else "iron_procurement"].enabled,"actual AI enables lowest eligible local route "+mode)
		...
		check(s.army.history.any(func(h): return h.action=="equipment_issue" and h.faction_id==f),"AI issues actual produced bundles "+mode)
		...
		records.append({"mode":mode,"production_gold":gold_production,"decisions":s.iron_ai,"supply_log":s.supply_transport.ai_log,"equipment":s.army.history,"national_before":before})
