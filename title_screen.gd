...
# 비워두면 DEFAULT_TITLE_MUSIC_PATH 경로에서 자동으로 불러옵니다.
@export var title_music: AudioStream
@export_range(-40.0, 6.0, 0.5) var title_music_volume_db: float = -8.0


# Inspector에 title_music을 따로 지정하지 않았을 때 자동으로 불러올 기본 경로.
# 업로드해주신 mp3를 이 경로에 넣어두면(res://assets/audio/title_theme.mp3)
# 코드 수정 없이 바로 재생됩니다.
const DEFAULT_TITLE_MUSIC_PATH: String = "res://assets/audio/title_theme.mp3"
const MUSIC_FADE_IN_SECONDS: float = 1.4
const MUSIC_FADE_OUT_SECONDS: float = 0.5
...
# Inspector의 title_music이 비어 있으면 DEFAULT_TITLE_MUSIC_PATH에서 자동으로
# 불러온다. mp3/ogg는 코드에서 loop를 켜줘야 타이틀 화면에서 계속 반복 재생된다.
func _start_music() -> void:
	if title_music == null:
		title_music = _load_default_title_music()

	if title_music == null:
		return

	_enable_stream_loop(title_music)
...
func _load_default_title_music() -> AudioStream:
	if not ResourceLoader.exists(DEFAULT_TITLE_MUSIC_PATH):
		return null

	var loaded_resource: Resource = load(DEFAULT_TITLE_MUSIC_PATH)

	if loaded_resource is AudioStream:
		return loaded_resource as AudioStream

	push_warning(
		"TitleScreen: %s는 오디오 리소스가 아닙니다."
		% DEFAULT_TITLE_MUSIC_PATH
	)
	return null
