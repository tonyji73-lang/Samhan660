...
static var OFFICERS: Dictionary = OfficerCatalog.legacy_database()
...
static func _prepare_scenario(base_scenario: Dictionary) -> Dictionary:
	...
	for faction: Dictionary in prepared_factions:
		faction["ruler_id"] = OfficerCatalog.ruler_id(str(scenario.id), str(faction.id))
		for entry: Dictionary in OfficerCatalog.scenario(str(scenario.id)).get("entries", []):
			if entry.officer_id == faction.ruler_id: faction["ruler"] = entry.display_name
...
static func get_featured_officers(
	...
	for entry: Dictionary in OfficerCatalog.scenario(scenario_id).get("entries", []):
		if entry.faction_id == faction_id: active_ids[entry.catalog_id] = true
	for officer_name_value: Variant in candidates:
		var officer_name: String = str(officer_name_value)
		var officer_id: String = OfficerCatalog.resolve(officer_name)
		if officer_id == OfficerCatalog.resolve(ruler_name) or not OFFICERS.has(officer_name) or not active_ids.has(officer_id):
			continue
...
static func get_age_text(person_name: String, scenario_year: int) -> String:
	var id: String = OfficerCatalog.resolve(person_name)
	if id.is_empty():
		return "나이 미상"
	var definition: Dictionary = OfficerCatalog.definitions()[id]
	var birth: Dictionary = definition.birth
	var death: Dictionary = definition.death
	if int(death.year) > 0 and scenario_year > int(death.year) and death.certainty == "confirmed":
		return "사망"
	if int(birth.year) > 0:
		return "%s%d세" % ["약 " if birth.certainty != "confirmed" else "", scenario_year-int(birth.year)]
	if int(birth.game_year) > 0 and scenario_year >= int(birth.game_year):
		return "게임상 %d세" % (scenario_year-int(birth.game_year))
	return "나이 미상"
