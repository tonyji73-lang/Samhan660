...
func refresh() -> void:
	...
	for envoy: Dictionary in campaign.get_diplomacy_envoys():
		...
		envoys.set_item_metadata(index, str(envoy.officer_id))
		if envoy.officer_id == selected_envoy_name:
			envoy_index = index
...
func _update_details() -> void:
	...
	details.text = "%s · 관계도 %d · %s\n현재 조약: %s\n선택 사절: %s" % [target if not target.is_empty() else "대상 세력 없음", int(relation.value), relation.status,
		" · ".join(display_treaties) if not display_treaties.is_empty() else "없음", campaign.get_officer(selected_envoy_name).get("name", "보유 사절 없음")]
	if pact:
